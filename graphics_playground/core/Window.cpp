...
void Window::mouseMoveCallback(GLFWwindow* window, double x_pos, double y_pos) {
	static_cast<Window*>(glfwGetWindowUserPointer(window))->mouse_callback_(x_pos, y_pos);
}

void Window::scrollCallback(GLFWwindow* window, double x_offset, double y_offset) {
	static_cast<Window*>(glfwGetWindowUserPointer(window))->scroll_callback_(x_offset, y_offset);
}
