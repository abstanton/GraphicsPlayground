...
namespace ecs {
	...
	public:
		...
		SingleComponentIterator<T> each() {
			return SingleComponentIterator<T>(this);
		}
	private:
	...
	struct SingleComponentIterator {
		...
		SingleComponentIterator(ComponentArray<T>* arr, size_t index) : base_array(arr), curr_index(index) {}

		reference operator*() const { return { base_array->index_entity_map_[curr_index] , base_array->components_[curr_index] }; }
		pointer operator->() { return { base_array->index_entity_map_[curr_index] , &base_array->components_[curr_index] }; }

		SingleComponentIterator& operator++() { curr_index++; return *this; }
		SingleComponentIterator* operator++(int) { SingleComponentIterator tmp = *this; ++(*this); return tmp; }

		friend bool operator==(const SingleComponentIterator& a, const SingleComponentIterator& b) { return a.curr_index == b.curr_index; }
		friend bool operator!=(const SingleComponentIterator& a, const SingleComponentIterator& b) { return a.curr_index != b.curr_index; }
	public:
		SingleComponentIterator begin() { return SingleComponentIterator(base_array); }
		SingleComponentIterator end() { return SingleComponentIterator(base_array, base_array->curr_size_); }
	private:
		size_t curr_index = 0;
