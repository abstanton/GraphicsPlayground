...
namespace gpu {	
	void GlBatch::vboBind() {
		...
		for (const auto& v : vert_buff_->vertex_attribs_) {
			glVertexAttribPointer(v.index, v.num, dataTypeToGlDataType(v.type), v.normalized, v.stride, v.offset);
	...
	GlBatch::GlBatch(VertexBuffer* vert_buff, IndexBuffer* index_buff) : Batch(vert_buff, index_buff) {
		glGenVertexArrays(1, &vao_id_);
		glBindVertexArray(vao_id_);
		vboBind();
		index_buff_->bind();
