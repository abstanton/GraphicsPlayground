...
namespace gpu {
	std::optional<GlShader> GlShader::fromFilepaths(const std::string& vertexShaderPath, const std::string& fragmentShaderPath) {
	...
	{
		glUniform2f(glGetUniformLocation(shaderProgram_, name.c_str()), value.x, value.y);
	}
	void GlShader::setVec3(const std::string& name, const glm::vec3& value) const
	{
		glUniform3f(glGetUniformLocation(shaderProgram_, name.c_str()), value.x, value.y, value.z);
	}
	void GlShader::setMat3(const std::string& name, const glm::mat3& value) const
	{
		glUniformMatrix3fv(glGetUniformLocation(shaderProgram_, name.c_str()), 1, GL_FALSE, glm::value_ptr(value));
	}
	void GlShader::setMat4(const std::string& name, const glm::mat4& value) const
	{
		glUniformMatrix4fv(glGetUniformLocation(shaderProgram_, name.c_str()), 1, GL_FALSE, glm::value_ptr(value));
	}
}